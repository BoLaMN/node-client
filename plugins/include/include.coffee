...
module.exports = ->
  ...
  @factory 'Inclusion', (injector) ->

    class IncludeScope
      constructor: (scope = {}) ->
        @_scope = utils.deepMerge {}, scope
        @_include = null

        if @_scope.include
          @_include = Inclusion.normalizeInclude @_scope.include
    ...
    class Inclusion
      ...
      include: (objects, include, options = {}) ->
        include = Inclusion.normalizeInclude include

        if not include
          return Promise.resolve objects

        Promise.each include, Inclusion.processIncludeItem objects, options
      ...
      @normalizeInclude: (include) ->
        if isString include
          ...
          include.map Inclusion.normalizeInclude
        else
          false

      @processIncludeItem: (objs, options) ->
        (include) ->
          subInclude = null
          scope = {}
          ...
          if not relation
            return Promise.reject new Error "Relation #{ relationName } is not defined for #{ @modelName } model"
